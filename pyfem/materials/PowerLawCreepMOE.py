...
class PowerLawCreepMOE( BaseMaterial ):
  ...
  def __init__ ( self, props ):
    ...
    self.setHistoryParameter( 'old_rohc' , zeros(1) )
    self.setHistoryParameter( 'old_rohw' , zeros(1) )
...
#------------------------------------------------------------------------------
  ...
  def getStress( self, kinematics):
    ...
    ol_rhoc = self.getHistoryParameter('old_rohc')
    ol_rhow = self.getHistoryParameter('old_rohw')
    ...
    if ol_rhoc > 8461801410123.313:
      ol_rhoc = 8461801410123.313

    if ol_rhow > 11999567054170.322:
      ol_rhow = 11999567054170.322

    ol_rhoc = 4461801410123.313
    ol_rhow = 999567054170.322
    ...
    if smises > ( 1.0 + self.tolerance ):
      ...
      input = np.array([smises, T, old_creep, ol_rhoc, ol_rhow, fl], dtype=float)
      ...
      while (abs(rhs) > self.tolerance or k==0):
        ...
        rhs   = output[0] * dt * 1e2 - deqpl
        J = MoEModel.Phi.eval_jacobian(input)
        jac = J[0,0] * self.eg3 * dt * 1e2 -1
        deqpl = deqpl - rhs/(jac) #Problem
        input = np.array([smises, self.Tmp, deqpl, ol_rhoc, ol_rhow, self.Flux])
        input = input.reshape(-1)
        output = MoEModel.Phi.eval(input)
        #print('------------------------------')
    ...
    self.setHistoryParameter( 'old_rohc' , ol_rhoc )
    self.setHistoryParameter( 'old_rohw' , ol_rhow )
